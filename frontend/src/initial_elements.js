...
// export const nodes = [{id:  '1' , type: 'input', data: { label: '  Autonomous Driving (AD) AI security research and the development of an open source evaluation platform called PASS to address the gap in scientific methodology. '}, position: { x :  0 , y: 0}},
// // {id:  '2' , data: { label: "  The authors analyze research papers related to semantic AI security in the context of autonomous driving, and identify scientific gaps and potential future directions. "}, position: { x :  500 , y: 0}},
// // {id:  '1' , data: { label: "  The security of Autonomous Driving (AD) AI stack and the need to study the security properties of downstream AI components. "}, position: { x :  250 , y: 0}},
// // {id:  '9' , data: { label: "  Autonomous Driving AI Research and Security "}, position: { x :  2250 , y: 0}},
// // {id:  '7' , data: { label: "  Attacks on Autonomous Driving Systems "}, position: { x :  1750 , y: 0}},
// // {id:  '5' , data: { label: " The common theme among the key points is the various methods of hacking autonomous driving (AD) systems. "}, position: { x :  1250 , y: 0}},
// // {id:  '4' , data: { label: "  Autonomous Driving (AD) AI Systems "}, position: { x :  1000 , y: 0}},
// // {id:  '3' , data: { label: "  Improving the robustness of AI components against attacks through defense methods. "}, position: { x :  750 , y: 0}},
// // {id:  '0' , data: { label: "  The lack of system-level evaluation and defense solutions in Autonomous Driving (AD) AI security research, and the need for more research in this area. "}, position: { x :  0 , y: 0}},
// // {id:  '6' , data: { label: "  The benefits of open-sourcing efforts within the security community, and potential solutions to encourage open sourcing practices. "}, position: { x :  1500 , y: 0}},
// ]
const clusterThemes = [
...
//   position: { x: 250, y: 0 },
// },
clusterThemes.forEach((clusterTheme) => {

  // console.log(clusterTheme.text)
  let nu = { id: `${counter}`, data: { label: clusterTheme.text }, position: { x: 100 * (counter + 1), y: 0 } }
  newNodesFromData.push(nu)
  counter += 1
})

// clusterSentences[0].forEach((cluster) => {
//   console.log(cluster)
let internalCounter = 0
clusterSentences[1].sentences.forEach((sentence) => {
  let nu = { id: `${1}${internalCounter}`, data: { label: sentence }, position: { x: 100 * (internalCounter + 1), y: 300 } }
  console.log(`${1}${internalCounter}`)
  newNodesFromData.push(nu)
  internalCounter += 1
})

internalCounter = 0
clusterSentences[2].sentences.forEach((sentence) => {
  let nu = { id: `${2}${internalCounter}`, data: { label: sentence }, position: { x: 100 * (internalCounter + 1), y: 600 } }
  console.log(`${2}${internalCounter}`)
  newNodesFromData.push(nu)
  internalCounter += 1
})
// })
...
export const nodes = newNodesFromData

let calcEdges = []
let childNodes = ['10','11','12','13','14','15','20','21','22','23','24','25','26']
childNodes.forEach((childNode) => {
  let source = childNode;
  let target = childNode[0];
  

  let ed = [
    {id: `e${childNode}`, source, target, label: nodes.find(x => x.id ===childNode)}
  ]
  calcEdges.push(ed)
})

// export const edges = calcEdges


export const edges = [
  { id: 'e1-0', source: '10', target: '1', },
  { id: 'e1-1', source: '11', target: '1' },
  { id: 'e1-2', source: '12', target: '1' },
  { id: 'e1-3', source: '13', target: '1' },
  { id: 'e1-4', source: '14', target: '1' },
  { id: 'e1-5', source: '15', target: '1' },

  { id: 'e2-0', source: '20', target: '2' },
  { id: 'e2-1', source: '21', target: '2' },
  { id: 'e2-2', source: '22', target: '2' },
  { id: 'e2-3', source: '23', target: '2' },
  { id: 'e2-4', source: '24', target: '2' },
  { id: 'e2-5', source: '25', target: '2' },
  // {
  //   id: 'e4-5',
  //   source: '4',
  //   target: '5',
